...
public:
    void testFieldExistenceQueries() {
        ...
        for (size_t chunk_idx = 0; chunk_idx < granular_data_.size(); ++chunk_idx) {
            ...
            for (const auto& field_name : test_fields) {
                ...
                // 使用正确的参数调用checkFieldExistenceAndType方法
                ...
                if (result.exists) {
                    std::cout << "  类型: " << static_cast<int>(result.actual_type) << std::endl;
    ...
    void testExactMatchQueries() {
        ...
        if (!granular_data_.empty()) {
            std::cout << "\n单块精确匹配查询 (user = \"postgres\"):" << std::endl;
            ...
            std::cout << "  找到记录数: " << single_result.count << std::endl;
            std::cout << "  查询时间: " << std::fixed << std::setprecision(2) 
                      << duration.count() << " ms" << std::endl;
            if (!single_result.error_message.empty()) {
                std::cout << "  错误: " << single_result.error_message << std::endl;
            } else {
                // 显示前几个匹配的记录
                std::cout << "  匹配记录示例:" << std::endl;
...
int main() {
    ...
    tester.testExactMatchQueries();  // 添加精确匹配查询测试
    // tester.testRangeQueries();       // 添加范围查询测试
    // tester.testAggregateQueries();   // 添加聚合查询测试
    // tester.testComplexQueries();     // 添加复杂查询测试
    // tester.testDirectGroupedAggregateQuery(); // 添加直接分组聚合查询测试
