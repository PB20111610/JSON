...
void FieldParser::collectAllFields(simdjson::dom::element node, 
    ...
    switch (node.type()) {
        ...
        default: {
            ...
            // String/LogType 冲突处理：避免同一字段名同时存在两种类型
            if (type == FieldType::LogType) {
                // 如果当前字段名已经有 String 类型，则将此次也按 String 处理
                FieldKey string_key{field_name, FieldType::String};
                if (all_fields.count(string_key) > 0) {
                    type = FieldType::String;
                }
            } else if (type == FieldType::String) {
                // 如果当前字段名已经有 LogType 类型，则将此次也按 LogType 处理
                FieldKey logtype_key{field_name, FieldType::LogType};
                if (all_fields.count(logtype_key) > 0) {
    ...
    for (simdjson::dom::element element : array) {
        ...
        switch (element.type()) {
            ...
            case simdjson::dom::element_type::NULL_VALUE:
                ...
                switch (element.type()) {
                    case simdjson::dom::element_type::STRING: {
                        // 对于字符串类型，需要进一步判断是 String 还是 LogType
                        Value value = extractValue(element);
                        std::string str_value = std::holds_alternative<std::string>(value) ? std::get<std::string>(value) : "";
                        ...
                        // 通过 manager 判断是 LogType 还是 String
                        if (manager.isLogTemplate(str_value)) {
                            type = FieldType::LogType;
                            // 检查是否已经有 String 类型
                            FieldKey string_key{field_name, FieldType::String};
                            if (all_fields.count(string_key) > 0) {
