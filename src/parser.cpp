...
DictType getDictType(const std::string& value) {
    ...
    if (std::regex_match(value, timestamp_pattern)) return DictType::TIMESTAMP_DICT;
    if (value.find(' ') != std::string::npos) return DictType::LOG_DICT;
    return DictType::VARIABLE_DICT;
}

// 简单属性-值对解析（适合结构化日志，无嵌套/转义）
static std::unordered_map<std::string, std::string> parseFlatJson(const std::string& jsonStr) {
