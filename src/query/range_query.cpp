...
// Implementation of executeRangeQuery
RecordQueryResult QueryEngine::executeRangeQuery(
    ...
    try {
        ...
        // Step 3: Perform layer value filtering for range in a single pass
        // Pass the pre-loaded dict_manager to avoid redundant dictionary decompression
        auto filter_result = this->filterLayerValuesInRange(field_name, expected_type, min_value, max_value, granular_data, 
                                                           dict_preloaded ? dict_manager.get() : nullptr);

        if (!filter_result.match_found) {
            // No matches found in the range, return empty result
        ...
        // Use the matched indices directly from the range filter result
        std::vector<size_t> range_matched_indices = filter_result.matched_indices;
        ...
        try {
            ...
            result.error_message = "Failed to decompress granular data for range query: " + std::string(e.what());
            return result;
        }
        
        if (!louds_trie) {
            result.error_message = "Failed to decompress granular data for range query";
            return result;
        }
        
        // Step 7: Get field order and use the already found target field
        const auto& final_field_order = louds_trie->getFieldOrder();
        result.chunks_accessed = 1; // We're accessing one chunk
        
        // Step 8: Now decompress the remaining dictionaries needed for reconstruction
        ...
        // Step 9: Process all matching indices to reconstruct their paths
        for (size_t matched_idx : range_matched_indices) {
...
RecordQueryResult QueryEngine::executeRangeQueryMultiBlock(
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        ...
        RecordQueryResult chunk_result = executeRangeQuery(field_name, min_value, max_value, expected_type, granular_data);
...
RecordQueryResult QueryEngine::executeRangeQueryMultiBlockParallel(
    ...
    if (thread_count <= 1 || chunks.empty()) {
        return executeRangeQueryMultiBlock(field_name, min_value, max_value, expected_type, chunks);
    }
    ...
    for (size_t group_idx = 0; group_idx < thread_count; ++group_idx) {
        ...
        futures.push_back(std::async(std::launch::async, [this, &field_name, &min_value, &max_value, expected_type, &chunks, group_indices = chunk_groups[group_idx]]() {
