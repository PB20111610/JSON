...
void testChunkedTypeAwareCompression() {
    ...
    try {
        ...
        if (!deserialized_blocks.empty()) {
            ...
            // Display field types
            std::cout << "Field types in first block:" << std::endl;
            for (const auto& field : first_block.field_order) {
                std::cout << "  " << field.name << ": " << fieldTypeToString(field.type) << std::endl;
            }
        }
        ...
        if (!deserialized_blocks.empty() && deserialized_blocks.size() >= 1) {
            try {
                ...
                std::ofstream reconstructed_file("chunked_type_aware_reconstructed.json");
        ...
        std::cout << "Chunked type-aware compression test completed successfully!" << std::endl;
